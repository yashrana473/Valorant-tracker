...
function App() {
  ...
  const getPlayerStats = async (e) => {
    ...
    try {
      const res = await axios.get(`http://localhost:5000/api/player/${ign}/${fixedTag}`);
  ...
  return (
    <div className="min-h-screen bg-valDark text-white flex flex-col items-center py-10 px-4">
      ...
      <form onSubmit={getPlayerStats} className="bg-gray-900 p-6 rounded-lg w-full max-w-lg border border-gray-800 mb-8">
      ...
      {profile && mmr && (
        <div className="w-full flex flex-col items-center pb-10 animate-fade-in">
          <div className="w-full max-w-lg bg-gray-900 border border-gray-800 rounded-lg overflow-hidden shadow-2xl">
